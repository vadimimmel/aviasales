...
const initialState = [
  { id: 'all', value: 'Все', status: true },
  { id: 'direct', value: 'Без пересадок', status: true },
  { id: 'oneLayover', value: '1 пересадка', status: true },
  { id: 'twoLayovers', value: '2 пересадки', status: true },
  { id: 'threeLayovers', value: '3 пересадки', status: true },
]

export function layoverFilter(state = initialState, action) {
  switch (action.type) {
    case SET_LAYOVER_FILTER:
      return changeFilter(state, action.filterID)
    default:
      return state
  }
}
